...
# define S_FORMAT_CLEAR "sed 's/^..//'"
	...
	FILE * exit_menu = popen (
		"echo \""
			...
			S_FORMAT (REBOOT) "\n"
			S_FORMAT (SHUTDOWN)
			"\" | dmenu -c -g 1 -l 5 -p exit: | " S_FORMAT_CLEAR
		,
		"r"
	);
	...
	else if (strcmp (exit_action, S_REBOOT) == 0) system ("dmenu -c -p 'Password:' -P | sudo -S reboot");
	else if (strcmp (exit_action, S_SHUTDOWN) == 0) system ("dmenu -c -p 'Password:' -P | sudo -S poweroff");
